 ...
 *	drawing 3d borders in OS/2 style.
 *
...
TkpGetBorder()
{
    OS2Border *borderPtr = (OS2Border *) ckalloc(sizeof(OS2Border));
...
TkpFreeBorder(borderPtr)
    TkBorder *borderPtr;
{
    OS2Border *os2BorderPtr = (OS2Border *) borderPtr;
    if (os2BorderPtr->light2ColorPtr) {
	Tk_FreeColor(os2BorderPtr->light2ColorPtr);
    }
    if (os2BorderPtr->dark2ColorPtr) {
	Tk_FreeColor(os2BorderPtr->dark2ColorPtr);
    }
}

/*
				 ...
				 * exterior. */
{
    ...
    int half;
    LONG windowHeight = TkOS2WindowHeight((TkOS2Drawable *)drawable);
	...
	case TK_RELIEF_RAISED:
	    ...
	    right = (leftBevel)
		? ((OS2Border *)borderPtr)->light2ColorPtr->pixel
		: ((OS2Border *)borderPtr)->dark2ColorPtr->pixel;
	    break;
    ...
    TkOS2FillRect(hps, x, y, half, height, left);
    TkOS2FillRect(hps, x+half, y, width-half, height, right);
				 ...
				 * exterior. */
{
	...
	if (x1 < x2) {
/*
	    TkOS2FillRect(hps, x1, windowHeight - y - 1, x2-x1, 1,
*/
	    TkOS2FillRect(hps, x1, windowHeight - y - 1, x2-x1, 1,
				 ...
				 * drawing. */
{
    ...
    if ((TkOS2IndexOfColor(borderPtr->bgColorPtr) == COLOR_3DFACE)
	|| (TkOS2IndexOfColor(borderPtr->bgColorPtr) == COLOR_WINDOW)) {
	...
	((OS2Border*)borderPtr)->dark2ColorPtr = Tk_GetColor(NULL, tkwin,
		Tk_GetUid("System3dDarkShadow"));
	((OS2Border*)borderPtr)->light2ColorPtr = Tk_GetColor(NULL, tkwin,
	...
	darkColor.green = 0;
	darkColor.blue = 0;
	((OS2Border*)borderPtr)->dark2ColorPtr = Tk_GetColorByValue(tkwin,
	    &darkColor);
	lightColor = *(borderPtr->bgColorPtr);
	((OS2Border*)borderPtr)->light2ColorPtr = Tk_GetColorByValue(tkwin, 
	    &lightColor);
    }
 ...
 *----------------------------------------------------------------------
 *
 * TkOS2GetBorderPixels --
 *
 *	This routine returns the 5 LONGs used to draw a given
 *	3d border.  
 *
 * Results:
