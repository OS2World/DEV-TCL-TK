 ...
 * tkOS2Embed.c --
 *
 *	This file contains platform specific procedures for OS/2 platforms
...
typedef struct Container {
    HWND parentHwnd;			/* OS/2 HWND to the parent window */
    TkWindow *parentPtr;		/* Tk's information about the container
					 * or NULL if the container isn't
					 * in this process. */
    HWND embeddedHwnd;			/* OS/2 HWND to the embedded window
					 */
 ...
 * TkpUseWindow --
 *
 *	This procedure causes a Tk window to use a given OS/2 handle
 *	for a window as its underlying window, rather than a new OS/2
 ...
 *	if the argument does not identify a legal OS/2 window handle),
				 ...
				 * for tkwin;  must be an integer value. */
{
    ...
    if (!WinIsWindow(TclOS2GetHAB(), hwnd)) {
     ...
     * TkOS2EmbeddedEventProc will process this message and set the embeddedHwnd
     * variable
     */

    containerPtr->embeddedPtr = winPtr;
    containerPtr->embeddedHwnd = NULLHANDLE;
...
#ifdef VERBOSE
    printf("TkpMakeContainer tkwin %x\n", tkwin);
#endif
...
MRESULT EXPENTRY
TkOS2EmbeddedEventProc(hwnd, message, param1, param2)
    HWND hwnd;
    ULONG message;
    MPARAM param1;
    MPARAM param2;
...
#ifdef VERBOSE
    printf("TkOS2EmbeddedEventProc hwnd %x, msg %x, p1 %x, p2 %x\n", hwnd,
           message, param1, param2);
    fflush(stdout);
#endif
    if (tsdPtr->firstContainerPtr == NULL) {
#ifdef VERBOSE
        printf("    firstContainerPtr == NULL !!\n");
        fflush(stdout);
#endif
return (MRESULT) 0;
	TkpMakeContainer(Tk_HWNDToWindow(hwnd));
    }
    ...
    for (containerPtr = tsdPtr->firstContainerPtr;
	    (containerPtr != NULL) && (containerPtr->parentHwnd != hwnd);
	    containerPtr = containerPtr->nextPtr) {
        if (containerPtr == NULL) {
            panic("TkOS2EmbeddedEventProc couldn't find Container record");
        }
    }
	 ...
	 * window.
	 */
	if (containerPtr->embeddedHwnd == NULLHANDLE) {
	    containerPtr->embeddedHwnd = (HWND)param1;
	} else {
	    return (MRESULT)0;
	}
	...
	break;
      case TK_GEOMETRYREQ:
	EmbedGeometryRequest(containerPtr, (int)param1, (int)param2);
	break;
    }
    return (MRESULT)1;
}
    ...
    int width, height;		/* Size that the child has requested. */
{
	...
	WinSetWindowPos(containerPtr->embeddedHwnd, NULLHANDLE, 0, 0,
    ...
    XEvent *eventPtr;			/* ResizeRequest event. */
{
	...
	if (containerPtr->embeddedHwnd) {
	    WinSetWindowPos(containerPtr->embeddedHwnd, NULLHANDLE,
...
TkpClaimFocus(topLevelPtr, force)
    ...
    int force;				/* One means that the container should
					 ...
					 * currently have it. */
{
    HWND hwnd = WinQueryWindow(Tk_GetHWND(topLevelPtr->window), QW_PARENT);
    WinSendMsg(hwnd, TK_CLAIMFOCUS, (MPARAM) force, MPVOID);
}

/*
 *----------------------------------------------------------------------
