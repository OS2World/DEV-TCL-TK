...
proc tkConsoleInit {} {
    ...
    if {[string compare $tcl_platform(platform) "windows"] && \
        [string compare $tcl_platform(platform) "os2"]} {
...
proc tkConsoleAbout {} {
    global tk_patchLevel tcl_platform
    if {[string match "os2" $tcl_platform(platform)]} {
        tk_messageBox -type ok -message "Tcl for OS/2 Presentation Manager
	...
	if {[llength $m] > 1} {
	    global tcl_platform
            if {[string match "windows" $tcl_platform(platform)] || \
                [string match "os2" $tcl_platform(platform)]} {
