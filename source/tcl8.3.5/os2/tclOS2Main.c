                                    ...
                                    * to execute commands. */
{
    ...
    if ((*appInitProc)(interp) != TCL_OK) {
        if (usePm) {
            sprintf(buffer, "application-specific initialization failed: %s\n",
                    interp->result);
            WinMessageBox(HWND_DESKTOP, NULLHANDLE, buffer, "Tclsh",
    ...
    if (tclStartupScriptFileName != NULL) {
        ...
        if (code != TCL_OK) {
            if (usePm) {
                sprintf(buffer, "%s\n", interp->result);
                WinMessageBox(HWND_DESKTOP, NULLHANDLE, buffer, "Tclsh", 0,
    ...
    if (usePm) {
        if (tsdPtr->tty && !Tcl_GetVar(interp, "NoTkCon", TCL_GLOBAL_ONLY) &&
