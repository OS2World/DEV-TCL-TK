...
TestgetplatformCmd(clientData, interp, argc, argv)
    ...
    char **argv;			/* Argument strings. */
{
    static char *platformStrings[] = { "unix", "mac", "windows", "os2" };
...
#  endif
	...
	*platform = TCL_PLATFORM_WINDOWS;
    } else if (strncmp(argv[1], "os2", length) == 0) {
	*platform = TCL_PLATFORM_OS2;
    } else {
        Tcl_AppendResult(interp, "unsupported platform: should be one of ",
		"unix, mac, windows or os2", (char *) NULL);
	return TCL_ERROR;
    }
    return TCL_OK;
}
